...
int authUser(SOCKET clnt, char* buff, char pass[][LEN_LOGPASS], char login[][LEN_LOGPASS], int counter) {
    ...
    if (!auth) {
        ...
        snprintf(buff, SIZE_BUF, "Password is incorrect. Authorization has been cancelled.\n");
        send(clnt, buff, strlen(buff), 0);
        return 0;
    }
    ...
    while (1) {
        ...
        do {
            ...
            if (buff[0] == '-') {
                ...
                case 0: // -stop Секретная команда
                    stopServ(listenSocket, clientSocket);
                    return 1;
                case 1: // -help Вывод помощи по командам
                    ...
                    send(clientSocket, buff, strlen(buff), 0);
                    break;
                case 2: // -login Авторизация клиента
                    auth_flag = authUser(clientSocket, (char*)&buff, password, login, log_counter);
                    break;
                case 3: // -reg Регистрация нового клиента
                    log_counter = regUser(clientSocket, (char*)&buff, password, login, log_counter);
                    break;
                case 4: // -list Вывод каталога файлов
                    printf("None.\n");
                    break;
                case 5: // -sendfile Отправка файла клиент -> сервер
                    sendFile(clientSocket, (char*)&buff);
                    break;
                case 6: // -getfile Отправка файла сервер -> клиент
                    printf("None.\n");
                    break;
                case 7: // -exit Прекращение сессии
                    printf("None.\n");
                    break;
                default: // -
